...
```powershell
Get-ChildItem "$HOME\.pi\agent\sessions" -Recurse -Filter *.jsonl | Where-Object { $_.FullName -like "*<project>*" } | Sort-Object LastWriteTime -Descending | Select-Object -First 5 -ExpandProperty FullName
```
...
uv run .\skills\session-reader\scripts\read_session.py $HOME\.pi\agent\sessions\<project>\subagent-artifacts\<hash>_worker.jsonl --mode overview

# Temp session file (may be cleaned up)
uv run .\skills\session-reader\scripts\read_session.py $env:TEMP\pi-subagent-session-<id>\run-0\<timestamp>.jsonl --mode overview
```
