...
Work interactively with the user in your Solo pane. Save the final plan to your pre-created Solo scratchpad and create Solo todos tagged: <tag>.`
})
```

Tell the user to continue the planning conversation in the planner pane. When Solo wakes the parent, read the planner scratchpad. If the scratchpad is not a final plan yet, tell the user the planner is still interactive and wait for their direction.
...
4. If blocked or missing context, update the todo with `todo_update` to add the missing examples or constraints, then rerun a worker.
