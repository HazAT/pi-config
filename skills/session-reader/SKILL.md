...
Parse pi session JSONL files into readable, structured output. Sessions live in `~/.pi/agent/sessions/<project>/` as `.jsonl` files. This is a local inspection workflow; only escalate to Codex if the session is large enough that better synthesis is clearly worth it.
