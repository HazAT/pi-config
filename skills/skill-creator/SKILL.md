...
For Claude Code-specific fields (`argument-hint`, `disable-model-invocation`, `context`, etc.), read `.\skills\skill-creator\references\claude-code-extensions.md`.
...
```markdown
Read `.\skills\skill-creator\references\topic-a.md` for details on [topic].
```
...
```powershell
uv run .\skills\skill-creator\scripts\quick_validate.py <path\to\skill-directory>
```
