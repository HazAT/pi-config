...
|-----------|-------------------|
| Creating documents | `${PI_SKILL_ROOT}/references/creation.md` |
| Editing documents | `${PI_SKILL_ROOT}/references/editing.md` |
```
...
2. Validate immediately: `uv run ${PI_SKILL_ROOT}/scripts/validate.py`
...
2. Validate the plan: `uv run ${PI_SKILL_ROOT}/scripts/validate_plan.py changes.json`
3. If validation fails, revise the plan and re-validate
4. Execute the plan: `uv run ${PI_SKILL_ROOT}/scripts/apply_changes.py changes.json`
5. Verify the result
```
