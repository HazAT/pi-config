...
- **Logo**: Read the official SVG from `skills/presentation-creator/references/sentry-logo.svg` (full wordmark) or `sentry-glyph.svg` (glyph only). Do NOT hardcode an approximation — always use the exact SVG paths from these files.

## Step 6: Common Slide Patterns

### Title Slide
Logo (from `skills/presentation-creator/references/sentry-logo.svg` or `sentry-glyph.svg`) + h1 + subtitle + author/date info.
