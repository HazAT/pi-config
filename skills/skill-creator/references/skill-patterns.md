...
- Scripts use PEP 723 inline metadata for dependencies:
  ...
  # ///
  ```
- Invoked with `uv run ${PI_SKILL_ROOT}/scripts/script_name.py`
...
- `disable-model-invocation: true` prevents automatic model invocation (appropriate for side-effect-heavy workflows)
- If `$ARGUMENTS` is absent from the content, arguments are appended as `ARGUMENTS: <value>`

**Note:** These features are runtime-specific extensions. Document them only when the target runtime supports them.
...
### Duplicating project convention files

**Problem:** SKILL.md repeats repo conventions already in AGENTS.md (commit format, PR process, etc.).

**Fix:** Reference AGENTS.md where needed. Skills should add domain knowledge, not repeat general conventions. Example: "Follow the commit conventions in AGENTS.md" instead of copying the entire commit format spec.
...
**Problem:** SKILL.md says `uv run ${PI_SKILL_ROOT}/scripts/tool.py` but doesn't document what arguments it takes or what it outputs.
...
```powershell
uv run ${PI_SKILL_ROOT}/scripts/tool.py --flag VALUE
```
Returns JSON:
```json
{"key": "value", "items": [...]}
...
**Fix:** Always use `${PI_SKILL_ROOT}/scripts/tool.py`. The variable resolves to the skill's directory regardless of where the agent runs from.
